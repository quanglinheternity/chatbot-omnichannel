        ...
        changes: [
          {
            ...
            value: {
              from,
              media: { id: "18055975949799859", media_product_type: "FEED" },
              id: "18202252114370779",
...
describe("Instagram Login comment username pass-through", () => {
  ...
  test("leaves the handle undefined when the payload has none", async () => {
    const add = await dispatch({ id: "2073295670063800" })

    const [, job] = add.mock.calls[0]
    expect(job.data.commentData.fromUsername).toBeUndefined()
    expect(job.data.commentData.fromName).toBe("2073295670063800")
  })
})
